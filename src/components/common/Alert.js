 ...
 * Props: type ('error'|'success'|'info'), message
 */
const Alert = ({ type = 'error', message }) => {
  ...
  const icons = {
    ...
    info: (
      <svg className="h-4 w-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1
          0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0
  ...
  return (
    <div role="alert" className={`flex items-start gap-2 rounded-lg px-4 py-3 text-sm ${styles[type]}`}>
      {icons[type]}
      <span>{message}</span>
    </div>
  );
